    ...
    namespace flowctl{namespace interactive
{
    ...
    BUNSAN_TYPED_STREAM_ENUM_INITIALIZED(CommandName,
    UnderlyingType,
    (
        (REQUEST, REQUEST_CLASS | 0x01),
        (EMPTY_REQUEST, REQUEST_CLASS | 0x02),
        (LAST_REQUEST, REQUEST_CLASS | 0x04),
        (EOF_REQUEST, EMPTY_REQUEST | LAST_REQUEST),

        (RESPONSE, RESPONSE_CLASS | 0x01),
        (LAST_RESPONSE, RESPONSE_CLASS | 0x02),
    ...
    typedef std::map<CommandName, std::string> CommandPack;
    ...
    constexpr bool isFlag(const CommandName name)
    {
        return name & FLAG_CLASS;
    }
